package me.xiaoying.turtle.bukkit.pluginmanager;

import me.xiaoying.turtle.api.core.TCore;
import me.xiaoying.turtle.api.pluginmanager.PluginManager;
import me.xiaoying.turtle.api.pluginmanager.PluginResponse;
import me.xiaoying.turtle.api.utils.YamlUtil;
import me.xiaoying.turtle.api.utils.ZipUtil;
...
public class SpigotPluginManager implements PluginManager {
    ...
    public PluginResponse loadPlugin(String pluginName) {
        for (File file : Objects.requireNonNull(TCore.getInstance().getDataFolder().getParentFile().listFiles())) {
            if (file.isDirectory())
                continue;
            ...
            try {
                TCore.getInstance().getPluginLoader().loadPlugin(file);
    ...
    public PluginResponse unloadPlugin(String pluginName) {
        PluginLoader pluginLoader = TCore.getInstance().getPluginLoader();
        try {
            ...
            for (PluginLoader loader : loaders) {
                ...
                TCore.getInstance().getPluginLoader().disablePlugin(plugin);
                return new PluginResponse(true, null);
            }
        } catch (Exception e) {
